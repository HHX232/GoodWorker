...
import { ChevronDownIcon, PlusIcon, XIcon, GripVerticalIcon } from 'lucide-react'
import {
  DndContext,
  closestCenter,
  PointerSensor,
...
const SortableOption = ({ id, index, option, onRemove }: SortableOptionProps) => {
  ...
  return (
    ...
    >
      ...
      {index === 0 && (
        <span className={styles.correct_badge}>верный ответ</span>
      )}
...
export const SelectTaskGapComponent = ({
  node,
  updateAttributes,
}: ReactNodeViewProps) => {
    ...
    <NodeViewWrapper as="span" className={styles.wrapper} contentEditable={false}>
      ...
      >
        <span className={styles.chip_label}>
          {isEmpty ? 'выбрать...' : options[0]}
        </span>
        <ChevronDownIcon
          size={11}
          className={`${styles.chip_chevron} ${open ? styles.chip_chevron_open : ''}`}
        />
      </button>
        ...
        <div ref={popoverRef} className={styles.popover}>
          ...
          {options.length > 0 && (
            <p className={styles.hint}>
              Перетащите чтобы изменить порядок · Первый — правильный
            </p>
          )}
        </div>
      )}
