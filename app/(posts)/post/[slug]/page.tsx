...
async function recordView(postId: string, userId: string, role: string): Promise<boolean> {
  ...
  if (role === 'TEACHER') {
    const alreadyViewed = await prisma.postView.findFirst({where: {postId, teacherId: userId}})
    if (alreadyViewed) return false
    await prisma.$transaction([
...
async function resolvePost(slugParam: string) {
  // New format: look up by slug field
  if (!UUID_RE.test(slugParam)) {
    const post = await withDbRetry(() => prisma.post.findUnique({
  ...
  // Legacy: param is a bare UUID — find and redirect to slug URL
  const post = await withDbRetry(() => prisma.post.findUnique({
    where: {id: slugParam},
    include: POST_INCLUDE,
  }))
...
export async function generateMetadata({params}: {params: Promise<{slug: string}>}): Promise<Metadata> {
  ...
  const post = await (UUID_RE.test(slug)
    ? prisma.post.findUnique({where: {id: slug}, select: {title: true, content: true, teacher: {select: {name: true}}, category: {include: {translations: true}}, slug: true}})
    : prisma.post.findUnique({where: {slug}, select: {title: true, content: true, teacher: {select: {name: true}}, category: {include: {translations: true}}, slug: true}})
  )
  if (!post) return {}
...
async function PostServerPage({params}: {params: Promise<{slug: string}>}) {
  const {slug} = await params
  const locale = (await cookies()).get('NEXT_LOCALE')?.value ?? 'ru'

  const [post, session] = await Promise.all([resolvePost(slug), auth()])
  ...
  if (session?.user?.id && session.user.role) {
    const incremented = await recordView(post.id, session.user.id, session.user.role)
  ...
  const [students, teachers, ratings] = await Promise.all([
    ...
    allAuthorIds.length
      ? prisma.postRating.findMany({where: {postId: post.id, authorId: {in: allAuthorIds}}, select: {authorId: true, stars: true}})
      : [],
  ])
