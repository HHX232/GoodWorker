...
## Сборка для main (без Кошелька)

Ветка `claude/tutor-files-vip-main` от `origin/main`: весь tutor-files без Кошелька. `billing.ts` — `STORAGE_BILLING_ENABLED = false`, `getStoragePricing() → null`; нет крона `storage-overage-billing`, нет `WalletSettings.storageOveragePriceCentsPerGbMonth` и `STORAGE_OVERAGE_DEBIT`. Квота из `StorageSettings` (по умолчанию 15 ГБ) — жёсткий потолок, предупреждение с 90%. Миграции tutor-files схлопнуты в одну `20260925210000_tutor_files`. При слиянии с веткой кошелька: взять её `billing.ts`, крон и её цепочку миграций (там те же таблицы — пересоздавать нельзя).
